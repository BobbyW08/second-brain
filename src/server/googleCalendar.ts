...
export const syncGoogleCalendar = createServerFn({ method: 'POST' })
  .inputValidator((data: { userId: string; accessToken: string; refreshToken: string }) => data)
...
export const createGoogleEvent = createServerFn({ method: 'POST' })
  .inputValidator((data: { block: { title: string; start_time: string; end_time: string }; accessToken: string; refreshToken: string }) => data)
...
export const updateGoogleEvent = createServerFn({ method: 'POST' })
  .inputValidator((data: { googleEventId: string; updates: { title?: string; start_time?: string; end_time?: string }; accessToken: string; refreshToken: string }) => data)
  .handler(async ({ data: { googleEventId, updates, accessToken, refreshToken } }) => {
  const auth = getOAuth2Client(accessToken, refreshToken)
  ...
  await calendar.events.patch({
    ...
    requestBody: {
      ...
      ...(updates.start_time && { start: { dateTime: updates.start_time } }),
...
export const deleteGoogleEvent = createServerFn({ method: 'POST' })
  .inputValidator((data: { googleEventId: string; accessToken: string; refreshToken: string }) => data)
