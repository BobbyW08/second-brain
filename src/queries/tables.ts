...
export type TableColumnType = "text" | "checkbox" | "select" | "date" | "number" | "url";
...
export function useCreateTable(userId: string) {
  ...
  return useMutation({
    ...
    onMutate: async (input) => {
      ...
      const previous = queryClient.getQueryData<TableSchema[]>(["tables", userId]);
...
export function useUpdateTable(userId: string) {
  ...
  return useMutation({
    ...
    onMutate: async ({ tableId, updates }) => {
      ...
      queryClient.setQueryData<TableSchema[]>(["tables", userId], (old) =>
        (old ?? []).map((t) =>
          t.id === tableId ? { ...t, ...updates, updated_at: new Date().toISOString() } : t,
        ),
      );
      return { previous };
    },
