...
export function buildTree(folders: Database["public"]["Tables"]["folders"]["Row"][], pages: Database["public"]["Tables"]["pages"]["Row"][]) {
  ...
  folders.forEach(folder => {
    ...
    if (folderNode && pagesByFolder[folder.id]) {
      ...
      const sortedPages = pagesByFolder[folder.id].sort((a, b) => (a.position || 0) - (b.position || 0));
  ...
  if (pagesByFolder.root) {
    ...
    const sortedRootPages = pagesByFolder.root.sort((a, b) => (a.position || 0) - (b.position || 0));
